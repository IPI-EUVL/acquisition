...
from euv_acquisition.sources.siglent import SIGLENT_CAPTURE_MODE
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a simulated Siglent sequence acquisition service."
    )
    parser.add_argument("--spool", required=True, help="Dedicated local simulated Siglent HDF5 spool directory.")
    parser.add_argument("--source-id", default="siglent-simulator", help="Stable simulated scope identity.")
    parser.add_argument("--sample-rate-hz", type=float, default=100_000_000.0)
    ...
    parser.add_argument("--baseline-volts", type=float, default=0.02)
    parser.add_argument("--noise-stddev-volts", type=float, default=0.002)
    parser.add_argument("--amplitude-volts", type=float, default=0.35)
    parser.add_argument("--center-seconds", type=float, default=1.5e-6)
    parser.add_argument("--width-seconds", type=float, default=0.35e-6)
    ...
    parser.add_argument("--log-host", default=os.environ.get("ECS_LOG_HOST", "127.0.0.1"))
...
def _build_server(args: argparse.Namespace, *, logger=None) -> AcquisitionServer:
    if args.points_per_frame <= 25:
        raise ValueError("Simulated Siglent points per frame must exceed the 25-sample baseline window.")
    ...
    pulse_config = SimulatedPulseConfig(
        seed=args.seed,
        baseline_volts=args.baseline_volts,
        noise_stddev_volts=args.noise_stddev_volts,
        amplitude_volts=args.amplitude_volts,
        center_seconds=args.center_seconds,
        width_seconds=args.width_seconds,
